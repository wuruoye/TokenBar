...
`Avg tok/s` divides generated output plus reasoning tokens by the summed active duration of the underlying model requests. Each duration begins when the request becomes eligible after a user prompt or tool result and ends when that request emits a tool call or final response, so queueing and time to first token remain included. Tool execution, polling, and other time between model requests stay in the displayed turn duration but are excluded from TPS. Repeated token-count snapshots that do not advance usage do not close or advance a request interval. Synchronized snapshots retain the timed-token numerator, duration denominator, and sample count so cross-device and date-window totals can recompute the rate without weighting untimed output. TokenBar omits TPS when a local transcript does not expose enough request-boundary timing to calculate it reliably.
