...
The health endpoint is unauthenticated. Deployments use a shared 32–512 character non-whitespace ASCII bearer token. Non-loopback client endpoints require HTTPS; HTTP is accepted only for loopback development/deployment. The client rejects all redirects, bypasses ambient forward proxies, and removes `TOKENBAR_SYNC_TOKEN` from the helper child process environment. No client or server component may log the shared token or snapshot payload. Mac bounds the streamed response before buffering more than 64 MiB; Windows and Linux also enforce the 64 MiB download limit while reading.
