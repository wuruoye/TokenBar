...
class SyncHTTPError(RuntimeError):
    def __init__(self, status: int):
        self.status = status
        super().__init__(f"sync server returned HTTP {status}")
...
def decode_snapshot(raw: bytes) -> dict[str, Any]:
    ...
    if not isinstance(timezone, str) or not timezone.strip() or len(timezone) > 128:
        raise ValidationError("activity snapshot must contain a non-empty timezone string")
    ...
    validate_activity_snapshot(value)
...
def collect_helper_snapshot(
    ...
    try:
        ...
        helper_environment.pop("TOKENBAR_SYNC_TOKEN", None)
...
def _read_token(token_file: str | None) -> str:
    token = os.environ.get("TOKENBAR_SYNC_TOKEN", "").strip()
    if token_file:
        try:
...
def _base_url(value: str) -> str:
    ...
    try:
        ...
        raise ValidationError("sync URL contains an invalid port") from error
    if parsed.scheme == "http" and (parsed.hostname or "").casefold() not in LOOPBACK_HOSTS:
        raise ValidationError("sync URL must use HTTPS except for loopback testing")
    return value.rstrip("/")
...
def extract_weekly_resets(
    ...
    *,
    ...
    def valid_reset_ms(value: Any, generated_at_ms: int) -> int | None:
        if (
            isinstance(value, int)
            and not isinstance(value, bool)
            and 0 < value
            and current_ms - MAX_WEEKLY_RESET_AGE_MS <= value <= generated_at_ms
        ):
            return value
        return None
    ...
    for entry in snapshots:
        ...
        if (
            ...
            or isinstance(generated_at_ms, bool)
            or generated_at_ms <= 0
            or generated_at_ms > current_ms
            or entry["snapshot"].get("generatedAtMs") != generated_at_ms
        ):
            continue
        ordered.append((received_at_ms, generated_at_ms, entry["snapshot"]))
    ordered.sort(key=lambda item: item[0], reverse=True)

    resets: dict[str, int] = {}
    for _received_at_ms, generated_at_ms, snapshot in ordered:
        ...
        if isinstance(sources, list):
            for source in sources:
                ...
                if not isinstance(weekly, dict):
                    continue
                reset = valid_reset_ms(weekly.get("startedAtMs"), generated_at_ms)
                if reset is not None:
...
def command_upload(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if error.status == 409:
            raise RuntimeError(
                "server rejected the snapshot as stale or conflicting; "
                "the next invocation will collect a fresh snapshot"
            ) from error
        raise
    print(
        "uploaded "
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    upload = subparsers.add_parser("upload", help="sanitize and upload a fresh snapshot")
